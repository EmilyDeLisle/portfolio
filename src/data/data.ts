...
Enter **Plant Pal (version 2)**. This version of **Plant Pal** is completely rebuilt and uses almost the entire suite of products from Firebase, including [Cloud Firestore](https://firebase.google.com/products/firestore), [Cloud Functions](https://firebase.google.com/products/functions), [Authentication](https://firebase.google.com/products/auth), [Storage](https://firebase.google.com/products/storage), and [Hosting](https://firebase.google.com/products/hosting). This time, I built the frontend in [TypeScript](https://www.typescriptlang.org/) using [React](https://reactjs.org/) and [Material-UI](https://material-ui.com/) for the user interface and [MobX](https://mobx.js.org/) for state management. Firebase provides the API to interact with the database in Cloud Firestore, and I added a couple Cloud Functions to do some additional backend work, like calculating watering and fertilizing intervals.

The new-and-improved **Plant Pal** now calculates watering *and* fertilizing dates ands intervals. Plants can be searched for and sorted in several different ways. The app sorts, by default, plants that require water most urgently based on their specific watering needs, rather than just by time since last watered. It can also sort the list of plants by name or date last fertilized or watered.
...
This app has made a world of difference in my plant-watering routine, and it feels really cool that I built it myself.`,
  ...
  },
  {
    ...
    description: `The **Pathfinder Youth Centre Society Database Application** was a collaborative student project I worked on alongside four other students for the BCIT Industry-Sponsored Student Project program in term 3 of the Computer Systems Technology program. This project was completed over a five-week period in April-May 2020, and was featured in July 2020 in [BCIT News](https://commons.bcit.ca/news/2020/07/issp-pathfinder-youth-centre-society/).
...
Because of these requirements, our team opted to go with storing the data in a NoSQL database with [Firebase Cloud Firestore](https://firebase.google.com/products/firestore) and secured it with [Firebase Authentication](https://firebase.google.com/products/auth). Firebase provided the hosting for the final project. The frontend was built with [React](https://reactjs.org/) and [Material-UI](https://material-ui.com/) for the user interface and [MobX](https://mobx.js.org/) for state management. We also used [Formik](https://formik.org/) with [Yup](https://github.com/jquense/yup) to manage the forms.
...
One part of the app that I designed that I am particularly proud of is that the app uses a single source of truth for its fields. The fields of the web form used by participants, the fields and participant data within the app, as well as the form initial values are all derived from a list of field objects. This makes the form fields easily modifyable and scaleable, allowing PYCS to update their form and change around the fields without needing to overhaul the form's code.
